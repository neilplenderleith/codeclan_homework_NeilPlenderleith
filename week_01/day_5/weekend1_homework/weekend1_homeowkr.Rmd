...
lets change the publication_date to a <date> type, and pull the year from it in 
a different column called year_written
...
books_clean <- books %>% 
  ...
  filter(!num_pages %in% c("eng", "en-US")) %>% # rows entered incorrectly - delete
...
# perhaps a better way to have filtered this would be by > 1000 reviews per books, say  
...
# should have added all the english type languages together - do if time
# use function recode
...
books_date <- books_clean %>% 
  ...
  summarise(rating_per_year = round(mean(average_rating), 2), 
            no_ratings_per_year = n()) %>% 
...
How many books are authored by multiple authors? Looking at the data multiple 
authors are separated by "/" character so if we check for that in each author 
and count it should give us the data.
...
# 9. Average time between books published by author? Can we do this?
...
# quite clean enough for this operation. Nor does my theory take into account
...
# 10. Any duplicate books?

From number 9 above it appears there are multiple copies of some books. 
Lets look and see
...
# This would be republished books as well as multi-language copies presumably

```

# 11. Best Sci-Fi book? 

Arg - there are no genres! What if we search by 10 famous sci-Fi authors?
This is a bit silly
```{r}
